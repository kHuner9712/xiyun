...
| Docker 镜像 | 当前 `mysql:8.0`，预部署目标为 `mysql:5.7.44`，SQL 兼容 5.7 |
...
| # | 事项 | 决策状态 | 建议 |
|---|------|---------|------|
| 1 | docker-compose.yml MySQL 镜像版本 | 已决策：本地保持 8.0，预部署使用 5.7.44 | SQL 已兼容 5.7，本地开发不强制降级 |
| 2 | docker-compose.yml 排序规则 | 已决策：统一为 `utf8mb4_general_ci` | `docker-compose.yml` 已设置为 `utf8mb4_general_ci` |
| 3 | invite 与 my-invite 页面定位重叠 | 未决策 | 一期保持现状，二期合并或明确区分 |
| 4 | 付费活动报名支付流程 | 已决策 | 一期活动报名仅免费，付费活动走商城下单 |
...
基于"先跑通核心链路，再补体验"的原则，以下为一期剩余未完成事项：

### 第一批：确认数据链路

1. **确认 goods.stage / goods.selling_point 数据来源** — 决定商品标签是否可用

### 第二批：补齐前后端闭环

2. **取消报名前端入口** — 后端已实现，前端无按钮
3. **报名页月龄 off-by-one 修复** — 自动计算与手动选择索引基准不一致
4. **活动详情 onShow 刷新** — 提升报名后体验
5. **用户反馈提交 API + 管理后台** — 补齐闭环
6. **报名状态确认（Admin）** — 运营需要确认操作

### 第三批：提审准备

7. **隐私协议内容定制** — 替换为母婴业务定制版
8. **小程序类目确认** — 母婴商城需选择正确类目
9. **全流程回归测试**
10. **提审提交**
