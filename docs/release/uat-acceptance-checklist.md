...
| 6.3 | 新用户通过邀请码注册 | P0 | 有邀请链接 | 1. 新用户打开邀请链接 2. 完成注册 | 注册成功，sxo_muying_invite_reward 表创建记录，trigger_event='register'，status='pending' | 半自动（DB 验证） |
| 6.4 | 首单奖励自动发放 | P0 | 被邀请人已注册且完成首单支付 | 1. 被邀请人下单并支付成功 2. 查看邀请人积分 | 邀请人积分增加，sxo_muying_invite_reward 表新增 trigger_event='first_order' 记录，status='granted' | 半自动（DB 验证） |
| 6.5 | 首单奖励配置读取 | P1 | 后台配置 muying_invite_first_order_reward=100 | 1. 查看邀请页奖励说明 | 展示"邀请好友首单奖励 100 积分" | 半自动 |
| 6.6 | 每日上限控制 | P1 | 后台配置每日上限 | 1. 同一邀请人当日多次触发奖励 | 超过上限后不再发放，记录 status='daily_limited' | 半自动（DB 验证） |
| 6.7 | 奖励记录列表 | P0 | 有奖励记录 | 1. 进入"我的邀请"页面 2. 查看奖励记录 | 展示奖励列表，含状态/时间/积分 | 手动 |
| 6.8 | 后台手动补发 | P1 | 有 pending 状态的奖励记录 | 1. 后台进入邀请奖励列表 2. 点击"补发" | 奖励发放成功，状态变为 granted | 半自动 |
| 6.9 | 后台撤销奖励 | P1 | 有 granted 状态的奖励记录 | 1. 后台点击"撤销" | 奖励撤销，积分扣回，状态变为 revoked | 半自动 |
