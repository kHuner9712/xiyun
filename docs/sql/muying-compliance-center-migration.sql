...
-- ============================================================
-- 一、合规中心菜单注册（id=770 起，避免与数据看板 id=760 冲突）
-- ============================================================
...
-- ============================================================
-- 四、旧环境修复：如果之前错误执行过 id=760-765 的合规中心菜单
-- 识别条件：id=760 的 control='Muyingcompliance'（而非 'Muyingstat'）
-- 修复动作：删除旧 760-765 → 重新插入 770-775
-- ============================================================
SET @old_760_control = '';
SELECT `control` INTO @old_760_control FROM `sxo_power` WHERE `id` = 760 LIMIT 1;

SET @sql = IF(@old_760_control = 'Muyingcompliance',
    'DELETE FROM sxo_role_power WHERE power_id BETWEEN 760 AND 765; DELETE FROM sxo_power WHERE id BETWEEN 760 AND 765 AND control = ''Muyingcompliance'';',
    'SELECT 1');
